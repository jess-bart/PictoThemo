package com.jessy_barthelemy.pictothemo.layouts;
...
public class FlowLayout extends LinearLayout {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            child = getChildAt(i);
            if (child.getVisibility() != GONE) {
                if(elementsWidth + child.getMeasuredWidth() >= width)
                    break;
                elementsWidth += child.getMeasuredWidth();
            }
        }

        int margin = (screenWidth - elementsWidth) / 2;
        int xpos = getPaddingLeft() + margin;
        int ypos = getPaddingTop();
